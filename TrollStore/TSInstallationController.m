...
- (void)installIpa:(NSString*)ipaPath withCallback:(void (^)(NSError* error))callback
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSError* error;
        ...
        NSString* tempPath = [NSTemporaryDirectory() stringByAppendingPathComponent:randomId];
        
        if(![self extractIpa:ipaPath toPath:tempPath error:&error])
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                callback(error);
            });
        ...
        // Find app bundle
        NSString* payloadPath = [tempPath stringByAppendingPathComponent:@"Payload"];
        NSArray* payloadContents = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:payloadPath error:&error];
        if(!payloadContents)
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                callback(error);
        ...
        for(NSString* payloadItem in payloadContents)
        {
            if([payloadItem.pathExtension isEqualToString:@"app"])
            {
                appBundlePath = [payloadPath stringByAppendingPathComponent:payloadItem];
                break;
            }
        }
        ...
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                callback([NSError errorWithDomain:@"com.opa334.trollstore" code:4 userInfo:@{NSLocalizedDescriptionKey : @"No app bundle found in IPA"}]);
            });
            return;
        }
        
        // Modify app with enhanced entitlements
        if(![TSAppModifier modifyAppEntitlements:appBundlePath withEntitlements:[TSAppModifier defaultEnhancedEntitlements]])
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                callback([NSError errorWithDomain:@"com.opa334.trollstore" code:5 userInfo:@{NSLocalizedDescriptionKey : @"Failed to modify app entitlements"}]);
            });
            return;
        }
        
        // Enable advanced features based on settings
        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
        
        if([defaults boolForKey:@"enable_jit"]) {
            [TSAppModifier enableJITForApp:appBundlePath];
        }
        
        if([defaults boolForKey:@"enable_debugging"]) {
            [TSAppModifier enableDebuggingForApp:appBundlePath];
        }
        
        if([defaults boolForKey:@"bypass_codesigning"]) {
            [TSAppModifier bypassCodeSigningForApp:appBundlePath];
        }
        
        // Install modified app
        NSString* targetPath = [@"/Applications" stringByAppendingPathComponent:[appBundlePath lastPathComponent]];
        
        if(![[NSFileManager defaultManager] copyItemAtPath:appBundlePath toPath:targetPath error:&error])
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                callback(error);
            });
        ...
        // Register app
        if(![self registerAppBundle:targetPath error:&error])
        {
